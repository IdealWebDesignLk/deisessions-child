<?php /* Template Name: Page_home_new_popup */ ?>
...
<video src="https://iwdsessions.com/wp-content/uploads/2022/10/Cross-Cultural-Communication.mp4" autoplay class="hidden" id="kd-test-autoplay"></video>
...
<div class="container-fluidx background-black">
    ...
    foreach ($catResults as $catResult) {
        ...
        if (intval($catResult->id) != 12) {
            ...
            // 					start if cluse that checks if the category is 12
    ?>
            <div class="kd-single-services-section">
                <h3 id="myList" class="hometitle"><?php echo $catResult->name; ?></h3>
                <br><br>
                <?php $results = $wpdb->get_results("SELECT * FROM wp_821991_amelia_services where status='visible' and categoryId='" . $catResult->id . "'");
                if (!empty($results)) { ?>
                            ...
                            <?php foreach ($results as $row) {
                                ...
                                foreach ($employee as $employeedetails) {
                                    ...
                                    if ($row->pictureFullPath != "") {
                                        ...
                                        $image_url = "http://tolqs.com/wp-content/uploads/2022/10/defaultimg.png";
                                    }

                                    if (get_field('approve', $worduser)) {
                            ?>

                                        <div class="swiper-slide kd-service-slide">
                                            <div class="kd-video-thumbnail">
                                                <div class="kd-thumbnnail" data-id="<?php echo $servicesingleid; ?>">
                        ...
                        <div class="kd-single-popup-inner">
                            <?php foreach ($results as $row) {
                                ...
                                foreach ($employee as $employeedetails) {
                                    ...
                                    $parameters = "?controls=1&showinfo=0&start=" . $row->videoStartTime . "&rel=0&loop=1";
...
<script>
    ...
    function loadYoutubeVideo(popup) {
        ...
        if (!videoLoaded.includes(popup.id)) {
            ...
            // let iframes = Array.from(popup.getElementsByClassName('kd-iframe-wrapper'))
            // iframes.
            if (videoImg.dataset.finalurl != '') {
                let iframeWrapper = document.createElement('div')
                iframeWrapper.classList.add('kd-iframe-wrapper')
                let iframe = `<iframe class="carouselvideo" width="278" height="154" src=${vidUrl} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`
    ...
    function loadPopup() {
        ...
        thumbnails.forEach(thumbnail => {
            ...
            thumbnail.addEventListener('mouseenter', () => {
                // 			console.log(getOffset(thumbnail).left)
                ...
                thumbnails.forEach(thmb => {
                    // thmb.parentElement.parentElement.style.width = `${kdCrWidth}px`
                    // console.log(`${kdCrWidth}px`)
                ...
                // thumbnail.parentElement.parentElement.style.width = `600px`
                ...
                if ((window.innerWidth - getOffset(thumbnail).left) < 400) {
